//removing header and trailer using zipWithIndex:
  def stripLines(headerLines:Int,footerLines:Int,filename:String,delimiter:String)={
      ...
      val file_rddx=file_rdd.zipWithIndex.map{case(pcontent,pindex)=>(pindex,pcontent)}.filter{case(pindex,pcontent)=>{if (headerLines>0) !((0 to (headerLines-1)).toList.contains(pindex)) else true} && 
